package me.molfordan.bridgefightplugin.task;
...
import me.molfordan.bridgefightplugin.region.FlagType;
...
public class EggBridgeTask implements Runnable {
    ...
    public EggBridgeTask(Player player, Egg egg, Plugin plugin) {
        ...
        BridgeFightPlugin.getInstance().getEggBridgeManager().registerTask(this);
    ...
    private void placeBlock(Location location) {
        ...
        if (BridgeFightPlugin.getInstance().getRegionManager().isDenied(location, FlagType.BUILD)) {
            return;
        }

        // 2. Check Arena build limit
        Arena arena = BridgeFightPlugin.getInstance().getArenaManager().getArenaByLocationIgnoreY(location);
    ...
    public void cancel() {
        ...
        BridgeFightPlugin.getInstance().getEggBridgeManager().unregisterTask(this);
